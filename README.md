...
| `camera.jma_nowcast_tile_x4`  | Camera | 監視範囲タイル ×4  — 監視円が画像幅の **1/4** （最ズームイン、3000×3000 px） |
...
3000×3000 px の PNG を返す Camera を 4 つ用意しています。各画像は
...
> 地図タイルは初回描画時にのみ取得され、以降はメモリにキャッシュされます。
