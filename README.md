...
4 段階のズーム（`×4` / `×8` / `×16` / `×32`）で同じ監視位置を撮ったような
1024×1024 px の PNG を返す Camera を 4 つ用意しています。各画像は
**監視位置が常に画像の中央**にあり、必要に応じて複数タイルを連結・クロップして
生成されます。

レイヤ構成:
- 下層: **GSI 淡色地図**（国土地理院、ズーム自動選択）
- 上層: **JMA HRPNS 降水ナウキャストの最新観測タイル**（半透明オーバーレイ）
- 最前面: **監視範囲の半径円** と **中心マーカー**
...
>
> GSI ベース地図は **アップサンプルを起こさない最小ズーム** を都度選択して
> 取得し、`/config/jma_nowcast_cache/gsi_pale/{z}/{x}/{y}.png` にディスク
> キャッシュされます。HA 再起動を跨いで再利用されるので、初回 (cold cache)
> のみ少し時間がかかりますが、2 回目以降は即時応答です。クリアしたい場合は
> このディレクトリを削除してください。
