...
# 各色バンドの「上限 (max) 値」で表現する規約 (JMA 気象庁 高解像度降水
# ナウキャスト公式凡例と一致):
#   薄水色 (0.1〜1 mm/h)  → 1.0
#   水色   (1〜5)         → 5.0
#   青     (5〜10)        → 10.0
...
#   橙     (30〜50)       → 50.0
#   赤     (50〜80)       → 80.0
#   紫     (80 以上)      → 上限なし (99.9 を代表値に)
# 凡例出典: https://www.jma.go.jp/bosai/nowc/ の凡例パネル
#
# 補足:
#   - RGB(255,255,255) は JMA タイルで alpha=0 (完全透明) = "雨なし"
#     (analyzer 側で alpha<50 は skip する)。
#   - RGB(242,242,255) は alpha=255 の不透明色で「意図的に描画された
#     弱雨マーカー」= 0.1〜1 mm/h 帯。
#   - RGB(0,200,50) / (0,130,30) の 2 色は JMA 公式 8 バンド凡例には
#     載っていないが実タイル中に存在する。標準色帯の間に位置する中間
#     色として、濃青 (20) と黄 (30) の間の 20〜25、25〜30 と推定。
JMA_PALETTE: list[tuple[tuple[int, int, int], float]] = [
    # 透明 (雨なし)
    ((255, 255, 255),   0.0),
    # 0.1〜1 mm/h band → 上限 1.0 (薄水色)
    ((242, 242, 255),   1.0),
    # 1〜5 mm/h band → 上限 5.0 (水色)
    ((160, 210, 255),   5.0),
    # 5〜10 mm/h band → 上限 10.0 (青)
    (( 33, 140, 255),  10.0),
    # 10〜20 mm/h band → 上限 20.0 (濃青)
    ((  0,  65, 255),  20.0),
    # 20〜25 mm/h 中間 (extra: 明るい緑、凡例外)
    ((  0, 200,  50),  25.0),
    # 25〜30 mm/h 中間 (extra: 濃緑、凡例外)
    ((  0, 130,  30),  30.0),
    # 20〜30 mm/h band → 上限 30.0 (黄)
    ((250, 245,   0),  30.0),
    # 30〜50 mm/h band → 上限 50.0 (橙)
    ((255, 153,   0),  50.0),
    # 50〜80 mm/h band → 上限 80.0 (赤)
    ((255,  40,   0),  80.0),
    # 80 mm/h以上 → 上限なし (紫) 代表値 99.9
    ((180,   0, 104),  99.9),
]
